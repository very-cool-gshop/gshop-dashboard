<script setup lang="ts">
  ...
  const columns: TableColumn<Product>[] = [
    ...
    },
    {
      ...
      header: '庫存',
      cell: ({ row }) =>
        h('span', { class: row.original.stock === 0 ? 'text-error' : '' }, row.original.stock)
    },
    {
      accessorKey: 'status',
      header: '狀態',
      filterFn: 'equals',
      cell: ({ row }) =>
        h(UBadge, { variant: 'subtle', color: statusColorMap[row.original.status] }, () => statusLabelMap[row.original.status])
    },
    {
      id: 'actions',
      cell: ({ row }) =>
  ...
  <UDashboardPanel id="products">
    <template #header>
      <UDashboardNavbar title="商品管理">
        ...
        <template #right>
          <UButton icon="i-lucide-plus" label="新增商品" />
        </template>
      </UDashboardNavbar>
    </template>
